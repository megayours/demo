...
output=$(chr deployment create \
  --network devnet1 \
  --blockchain mega_chain \
  --secret .secret \
  -y)

# Extract the chain ID from the output
chain_id=$(echo "$output" | grep -oP '(?<=mega_chain: x")[^"]*')
...
if [ -f "chromia.yml" ]; then
  yq e ".deployments.devnet1.chains.mega_chain = \"$chain_id\"" -i chromia.yml
  ...
  devnet1:
    chains:
      mega_chain: \"$chain_id\"" > chromia.yml
fi
